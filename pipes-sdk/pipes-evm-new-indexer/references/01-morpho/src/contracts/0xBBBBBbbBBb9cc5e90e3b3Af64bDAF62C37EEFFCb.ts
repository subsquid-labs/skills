...
export const events = {
  AccrueInterest: event(
    ...
    { id: indexed(p.bytes32), prevBorrowRate: p.uint256, interest: p.uint256, feeShares: p.uint256 },
  ),
  Borrow: event(
  ...
  EnableIrm: event('0x590e04cdebeccba40f566186b9746ad295a4cd358ea4fefaaea6ce79630d96c0', 'EnableIrm(address)', {
    irm: indexed(p.address),
  }),
  EnableLltv: event('0x297b80e7a896fad470c630f6575072d609bde997260ff3db851939405ec29139', 'EnableLltv(uint256)', {
    lltv: p.uint256,
  }),
  FlashLoan: event(
    '0xc76f1b4fe4396ac07a9fa55a415d4ca430e72651d37d3401f3bed7cb13fc4f12',
    'FlashLoan(address,address,uint256)',
    { caller: indexed(p.address), token: indexed(p.address), assets: p.uint256 },
  ),
  IncrementNonce: event(
    '0xa58af1a0c70dba0c7aa60d1a1a147ebd61000d1690a968828ac718bca927f2c7',
    'IncrementNonce(address,address,uint256)',
    { caller: indexed(p.address), authorizer: indexed(p.address), usedNonce: p.uint256 },
  ),
  Liquidate: event(
    ...
    'Liquidate(bytes32,address,address,uint256,uint256,uint256,uint256,uint256)',
    {
      ...
      borrower: indexed(p.address),
      repaidAssets: p.uint256,
  ...
  SetAuthorization: event(
    ...
    {
      caller: indexed(p.address),
      authorizer: indexed(p.address),
      authorized: indexed(p.address),
      newIsAuthorized: p.bool,
    },
  ),
  SetFee: event('0x139d6f58e9a127229667c8e3b36e88890a66cfc8ab1024ddc513e189e125b75b', 'SetFee(bytes32,uint256)', {
    id: indexed(p.bytes32),
    newFee: p.uint256,
  }),
  SetFeeRecipient: event(
    '0x2e979f80fe4d43055c584cf4a8467c55875ea36728fc37176c05acd784eb7a73',
    'SetFeeRecipient(address)',
    { newFeeRecipient: indexed(p.address) },
  ),
  SetOwner: event('0x167d3e9c1016ab80e58802ca9da10ce5c6a0f4debc46a2e7a2cd9e56899a4fb5', 'SetOwner(address)', {
  ...
  SupplyCollateral: event(
    ...
    { id: indexed(p.bytes32), caller: indexed(p.address), onBehalf: indexed(p.address), assets: p.uint256 },
  ),
  Withdraw: event(
...
export const functions = {
  ...
  accrueInterest: fun('0x151c1ade', 'accrueInterest((address,address,address,address,uint256))', {
    marketParams: p.struct({
      loanToken: p.address,
      collateralToken: p.address,
      oracle: p.address,
  ...
  borrow: fun(
    ...
    { _0: p.uint256, _1: p.uint256 },
  ),
  createMarket: fun('0x8c1358a2', 'createMarket((address,address,address,address,uint256))', {
    marketParams: p.struct({
      loanToken: p.address,
      collateralToken: p.address,
      oracle: p.address,
  ...
  flashLoan: fun('0xe0232b42', 'flashLoan(address,uint256,bytes)', {
    token: p.address,
    assets: p.uint256,
    data: p.bytes,
  }),
  idToMarketParams: viewFun(
    '0x2c3c9157',
    'idToMarketParams(bytes32)',
    { _0: p.bytes32 },
    { loanToken: p.address, collateralToken: p.address, oracle: p.address, irm: p.address, lltv: p.uint256 },
  ),
  ...
  market: viewFun(
    ...
    { _0: p.bytes32 },
    {
      totalSupplyAssets: p.uint128,
      totalSupplyShares: p.uint128,
      totalBorrowAssets: p.uint128,
      totalBorrowShares: p.uint128,
      lastUpdate: p.uint128,
      fee: p.uint128,
    },
  ),
  ...
  position: viewFun(
    ...
    { _0: p.bytes32, _1: p.address },
    { supplyShares: p.uint256, borrowShares: p.uint128, collateral: p.uint128 },
  ),
  repay: fun(
    '0x20b76e81',
  ...
  setAuthorization: fun('0xeecea000', 'setAuthorization(address,bool)', {
    authorized: p.address,
    newIsAuthorized: p.bool,
  }),
  setAuthorizationWithSig: fun(
    '0x8069218f',
    'setAuthorizationWithSig((address,address,bool,uint256,uint256),(uint8,bytes32,bytes32))',
    {
      authorization: p.struct({
  ...
  setFee: fun('0x2b4f013c', 'setFee((address,address,address,address,uint256),uint256)', {
    marketParams: p.struct({
      loanToken: p.address,
      collateralToken: p.address,
      oracle: p.address,
